...
def save(tf_session, model_folder, cp_name, scope=None):
  ...
  if scope is None:
    ...
    var_list = tensorflow.compat.v1.get_collection(tensorflow.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope=scope)
    saver = tf.train.Checkpoint(var_list=dict([(var.name, var) for var in var_list])) 
  save_path = saver.save(file_prefix=os.path.join(model_folder, '{0}.ckpt'.format(cp_name)))
...
def load(tf_session, model_folder, cp_name, scope=None, verbose=False):
  ...
  """
  # Load model proper
  load_path = os.path.join(model_folder, '{0}.ckpt-1'.format(cp_name))
  ...
  initial_vars = set(
      [v.name for v in tensorflow.compat.v1.get_default_graph().as_graph_def().node])
