...
class ScaledDotProductAttention(tf.keras.layers.Layer):
  ...
  def __init__(self, attn_dropout=0.0):
    ...
    self.activation = tf.keras.activations.softmax #Activation('softmax')
  ...
  def __call__(self, q, k, v, mask):
    ...
    #   attn = Add()([attn, mmask]) TODO: dobbeltsjekk, chatGPT claimer at det er good
    
    attn = tf.einsum('bij,bjk->bik', q, tf.transpose(k, perm=[0, 2, 1])) / temper  # shape=(batch, q, k)
    if mask is not None:
      mmask = (-1e+9) * (1. - tf.cast(mask, tf.float32))  # setting to infinity
      attn += mmask
...
class TemporalFusionTransformer(object):
  ...
  def __init__(self, raw_params, custom_name=None, formatter=None):
  ...
  def get_tft_embeddings(self, all_inputs):
    ...
    known_combined_layer = tf.keras.backend.stack(
        known_regular_inputs + known_categorical_inputs, axis=-1) # Index out of range error here if both lists are empty
  ...
  def _batch_data(self, data):
    ...
    data_map = {}
    for id_val, sliced in data.groupby(id_col):
      col_mappings = {
          'identifier': [id_col],
          'time': [time_col],
          'outputs': [target_col],
      ...
      for k in col_mappings:
        ...
        if arr is not None:
          ...
          if k == 'identifier':
            print(f"None123. Id: {id_val}, Length: {len(sliced[cols])}")
  ...
  def build_model(self):
    ...
    with tensorflow.compat.v1.variable_scope(self.name):
  ...
  def fit(self, train_df=None, valid_df=None):
    ...
    # If use_multiprocessing is True and workers > 0, then keras will create multiple (number = workers)
    # processes to run simultaneously and prepare batches from your generator/sequence. They will try 
    # to keep the queue of batches ready for training up to max_queue_size.
  ...
  def get_attention(self, df):
    ...
    def get_batch_attention_weights(input_batch):
      ...
      for k in self._attention_components:
        attention_weight = tensorflow.compat.v1.keras.backend.get_session().run(
  ...
  def get_hyperparm_choices(cls):
    ...
    return {
        'dropout_rate': [0.15, 0.3, 0.5, 0.7, 0.85],
        'hidden_layer_size': [40, 80, 160, 240, 320],
        'minibatch_size': [32, 64, 128, 256],
        'learning_rate': [1e-4, 1e-3, 1e-2],
        'max_gradient_norm': [0.01, 1.0, 10.0],
        'num_heads': [1, 4],
        'stack_size': [1],
    }
  ...
  def train(self, x_train, y_train, x_val, y_val, pred_name="return_1w"):
    train_df = x_train.copy()
    valid_df = x_val.copy()
    train_df[pred_name]=y_train
    valid_df[pred_name]=y_val
    
    if self.formatter._real_scalers is None or self.formatter._cat_scalers is None:
      self.formatter.set_scalers(train_df)
    #TODO: Skal den ta inn noe av train i valid settet?
    self.fit(train_df=self.formatter.transform_inputs(train_df), valid_df=self.formatter.transform_inputs(valid_df))


  def forward(self, X: pd.DataFrame, return_targets=True):
    out_map = self.predict(self.formatter.transform_inputs(X.sort_values(by=['gvkey', 'datadate'], ascending=[True, True])), return_targets=return_targets)
    return self.formatter.format_predictions(out_map['p50']), self.formatter.format_predictions(out_map['targets'])
    
  def get_input_size(self):
    return self.num_encoder_steps
...
def append_to_sets(test_df: pd.DataFrame, train_df: pd.DataFrame, number_of_dates_to_append: int, date_col='datadate'):
  unique_dates = sorted(train_df[date_col].unique())
  latest_dates = unique_dates[-number_of_dates_to_append:]
  latest_dates_train = train_df[train_df[date_col].isin(latest_dates)]
  test_df_appended = pd.concat([latest_dates_train, test_df], ignore_index=True)
  return test_df_appended